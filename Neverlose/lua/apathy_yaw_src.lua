...
local default_config = {author = {"apathy team"},date = {"-"},n = {"Default"},cfg = {network.get("https://text-host.ru/raw/default-config-apathy")}}
...
local menu = {
    ...
    mods = {
        anim_breakers = tabs.misc.mods:switch(theme_color() .. icons.danger .. "\aDEFAULT   Anim breakers", false, "", function(gear)
            return {
                body_lean = gear:combo("Body lean", {"Disabled", "Neverlose", "Default", "Extreme"}),
...
function f_load_cfg()
    if(menu.config_system.cfg_list:get() == 1) then
        pui.load(json.parse(base64.decode(network.get("https://text-host.ru/raw/default-config-apathy"))))
    else
...
function inside_updateCSA(thisptr, edx)
    ...
    if(menu.mods.anim_breakers:get()) then
        ...
        if (menu.mods.anim_breakers.body_lean:get() ~= "Disabled") then
            ...
            ffi.cast('CAnimationLayer**', ffi.cast('uintptr_t', thisptr) + 0x2990)[0][12].m_flWeight = 0
        end
